...
def parse_args():
    ...
    parser.add_argument(
        "--dataset", help="cityscapes_instance_only", default="cityscapes_instance_only", type=str
    )
    ...
    parser.add_argument(
        "--coarse", help="include coarse annotations in conversion", action="store_true"
    )
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
...
def convert_cityscapes_instance_only(data_dir, out_dir, use_coarse=False):
    ...
    if use_coarse:
        sets += ["gtCoarse_train", "gtCoarse_val", "gtCoarse_train_extra"]

        ann_dirs += ["gtCoarse/train", "gtCoarse/val", "gtCoarse/train_extra"]
        ...
        for root, _, files in os.walk(ann_dir):
            for filename in files:
                if filename.endswith(ends_in % data_set.split("_")[0]):
                    ...
                    fullname = os.path.realpath(os.path.join(root, image["seg_file_name"]))
...
if __name__ == "__main__":
    ...
    if args.dataset == "cityscapes_instance_only":
        convert_cityscapes_instance_only(args.datadir, args.outdir, args.coarse)
    else:
