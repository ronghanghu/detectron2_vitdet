...
def get_dataset():
    ...
    dataset_valminusminival = COCODataset(
        valminusminival_ann_file, val_data_dir, ImageTransform()
    )
...
class RPNModel(torch.nn.Module):
    def __init__(self, pretrained_path):
        ...
        from torch_detectron.core.rpn_losses import (
            RPNLossComputation,
            RPNTargetPreparator,
        )
    ...
    def forward(self, images, targets=None):
        ...
        return dict(loss_objectness=loss_objectness, loss_box_reg=loss_box_reg)
...
def get_optimizer(model):
    ...
    optimizer = torch.optim.SGD(
        params, lr, momentum=momentum, weight_decay=weight_decay
    )
