...
def train_one_epoch(model, optimizer, data_loader, epoch, args):
    ...
    for i, batch in enumerate(data_loader):
        ...
        for lvl in range(len(rpn_scores)):
            ...
            # print(is_useful.float().sum().item())
        ...
        # print([c.item() for c in rpn_classif_losses])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr-steps', default=1, nargs='+', type=int)
    ...
    if args.ar_group:
        ...
        """
        for didx in range(len(dataset.datasets)):
            for i in range(len(dataset.datasets[didx])):
                img_data = dataset.datasets[didx].get_img_info(i)
                aspect_ratio = float(img_data['height']) / float(img_data['width'])
                aspect_ratios.append(aspect_ratio)
    ...
    from lib.fpn import C2ResNetFPN
    if False:
        ...
        model = C2ResNetFPN(data, rpn_batch_size=rpn_batch_size, batch_size=batch_size)
        ...
        if True and args.local_rank == 0:
            if epoch == args.epochs and True:# args.evaluate:
                if args.rpn_only:
                    ...
                    from testing_fpn import evaluate
