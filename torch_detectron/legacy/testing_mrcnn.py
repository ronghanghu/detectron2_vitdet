...
def evaluate(model, data_loader, args):
    ...
    for i, batch in tqdm(enumerate(data_loader)):
        ...
        for im in range(imgs.shape[0]):
            ...
            with torch.no_grad():
                masks = model.predict_masks(features, torch.from_numpy(final_boxes).cuda())
                masks = masks.sigmoid()
            # normalize boxes
            for l in im_results:
                if len(l) == 0:
            ...
            for i in range(len(im_results)):
                ...
                t = len(im_results[i])
                selected_masks[i] = masks[counter:(counter + t), i]
                counter += t
    ...
    torch.save(mask_results, join(args.save, 'mask_results_v2.pth'))
    

    #results = torch.load(join(args.save, 'results.pth'))
    #mask_results = torch.load(join(args.save, 'mask_results.pth'))
    ...
    for cl, per_cat_results in enumerate(results):
        ...
        for im, dets in enumerate(per_cat_results):
            ...
            img_masks = [paste_mask_in_image(m, boxes[iidx], height, width).numpy() for iidx, m in enumerate(masks)]
    ...
    res_file_mask = join(args.save, 'json_results_mask.json')
    ...
    torch.save(coco_eval_m, join(args.save, 'computed_results_mask.pth'))
