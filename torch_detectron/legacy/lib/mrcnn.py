...
class MRCNN_FPN(nn.Module):
    def __init__(self, **kwargs):
        super(MRCNN_FPN, self).__init__()
    ...
    def compute_losses(self, images, targets):
        (scores, box_deltas, rpn_scores, rpn_box_deltas, rpn_labels, rpn_box_targets,
                ...
                bbox_inside_weights, bbox_outside_weights, num_pos) = model(
                        imgs, im_shape, gt_boxes, img_idx_gt, gt_labels)
        ...
        return loss_classif, loss_box_reg, rpn_classif_losses, rpn_losses_box_reg
    ...
    def forward(self, imgs, im_shape, gt_boxes=None, img_idx_gt=None, gt_labels=None):
        ...
        for lvl, rpn_score in enumerate(rpn_scores):
            ...
            a, i, ii = self.rpn._generate_anchors(map_shape, im_shape, lvl)
        ...
        with torch.no_grad():
            ...
            boxes, selected_scores, idxs = self.rpn.select_top(all_anchors, rpn_logits, rpn_box_deltas, im_shape)
        ...
        for c, (feature, b, i, sc) in enumerate(zip(features[:-1], boxes[:-1], idxs[:-1], (1. / 4, 1. / 8, 1. / 16, 1. / 32))):
            ...
            """
            pooled_img2 = roi_align(feature, rois, (14, 14), sc, 2)
            pooled_imgs_mask.append(pooled_img2)
            """

        pooled_imgs = torch.cat(pooled_imgs, dim=0)
        """
        pooled_imgs_mask = torch.cat(pooled_imgs_mask, dim=0)
        """
        ...
        """
        masks = self._masker(pooled_imgs_mask)#[rois_idx_restore]
        """
        ...
        #return scores, box_deltas, rpn_scores, rpn_box_deltas, boxes, all_anchors, img_idx_proposal, inds_inside, idxs, masks
        return scores, box_deltas, rpn_scores, rpn_box_deltas, boxes, all_anchors, img_idx_proposal, inds_inside, idxs, features
        # return scores, box_deltas, rpn_scores, rpn_box_deltas, boxes, all_anchors, img_idx_proposal, inds_inside, idxs, bb, pp

    # TODO: only work for non-batch
    def predict_masks(self, features, boxes):
        scales = (1. / 4, 1. / 8, 1. / 16, 1. / 32)
        lvl_min = 2
        lvl_max = 5
        lvls = map_rois_to_fpn_levels(boxes, lvl_min, lvl_max)
        per_scale_boxes = []
        rois_idx_order = []
        for lvl in range(lvl_min, lvl_max + 1):
            idx_lvl = torch.nonzero(lvls == lvl)
            idx_lvl = idx_lvl.squeeze(1) if idx_lvl.numel() else idx_lvl
            per_scale_boxes.append(boxes[idx_lvl])
            rois_idx_order.append(idx_lvl)
        _, rois_idx_restore = torch.sort(torch.cat(rois_idx_order, 0))
        pooled_feats = []
        for c, (feat, box, scale) in enumerate(zip(features[:-1], per_scale_boxes, scales)):
            idxs = box.new(box.shape[0], 1).zero_()
            rois = torch.cat((idxs, box), dim=1)
            pooled_feat = roi_align(feat, rois, (14, 14), scale, 2)
            pooled_feats.append(pooled_feat)
        pooled_feats = torch.cat(pooled_feats, dim=0)
        masks = self._masker(pooled_feats)
        masks = masks[rois_idx_restore]
        return masks
    ...
    def prepare_targets(self, proposals, gt_boxes, img_idx_proposal, img_idx_gt, gt_labels):
        ...
        fpn_idxs = torch.tensor([i for i, x in enumerate(img_idx_proposal) for _ in range(x.numel())], dtype=img_idx_proposal[0].dtype)
...
class C2MRCNNResNetFPN(MRCNN_FPN, ResNet50_FPN):
    def __init__(self, weights=None, num_classes=81, rpn_batch_size=None, batch_size=None, rpn_only=None):
        super(C2MRCNNResNetFPN, self).__init__(num_classes=num_classes, rpn_batch_size=batch_size, batch_size=batch_size, rpn_only=rpn_only)
...
def log2(x):
    return torch.log(x) / torch.log(x.new_tensor(2))
...
def map_rois_to_fpn_levels(rois, k_min, k_max):
    ...
    # Eqn.(1) in FPN paper
    target_lvls = torch.floor(lvl0 + log2(s / s0 + 1e-6))
    target_lvls = torch.clamp(target_lvls, min=k_min, max=k_max)
    return target_lvls


if __name__ == "__main__":
    m = C2ResNetFPN('/private/home/fmassa/github/detectron.pytorch/torch_detectron/mrcnn-fpn50.pkl')
