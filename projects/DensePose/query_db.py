...
class PrintAction(EntrywiseAction):
    ...
    def add_parser(cls: type, subparsers: argparse._SubParsersAction):
        parser = subparsers.add_parser(
            cls.COMMAND,
            help='Output selected entries to stdout. ')
...
class ShowAction(EntrywiseAction):
    ...
    def _extract_data_for_visualizers_from_entry(
        ...
        for vis_spec in vis_specs:
            datas.append(
                bbox_list if 'bbox' == vis_spec
                else (bbox_list, dp_list))
        return datas
...
def setup_dataset(dataset_name):
    ...
    stop = timer()
    logger.info('Loaded dataset {} in {:.3f}s'.format(
        dataset_name, stop - start))
    return dataset
