...
class BoundingBoxVisualizer(object):
    ...
    def visualize(self, image_bgr, boxes_xywh):
        for bbox_xywh in boxes_xywh:
            image_bgr = self.rectangle_visualizer.visualize(
                image_bgr, bbox_xywh)
        return image_bgr
...
class ScoredBoundingBoxVisualizer(object):
    ...
    def __init__(
            self, min_score=0, bbox_visualizer_params=None,
            score_visualizer_params=None):
    ...
    def visualize(self, image_bgr, scored_bboxes):
        ...
        for i, box_xywh in enumerate(boxes_xywh):
            ...
            if score_i >= self.min_score:
                image_bgr = self.visualizer_bbox.visualize(
                    image_bgr, box_xywh)
