...
from .base import MatrixVisualizer, PointsVisualizer


class DensePoseResultsVisualizer(object):

    def __init__(
            self, data_extractor, segm_extractor, inplace=True,
            cmap=cv2.COLORMAP_PARULA, alpha=0.7, val_scale=1.0):
    ...
    def visualize(self, image_bgr, densepose_result: DensePoseResult):
...
class DensePoseResultsSegmentationVisualizer(DensePoseResultsVisualizer):

    def __init__(self, inplace=True, cmap=cv2.COLORMAP_PARULA, alpha=0.7):
        super(DensePoseResultsSegmentationVisualizer, self).__init__(
            _extract_i_from_iuvarr, _extract_i_from_iuvarr, inplace, cmap,
            alpha, val_scale=255.0 / (DensePoseDataRelative.N_BODY_PARTS + 1))
...
class DensePoseResultsUVisualizer(DensePoseResultsVisualizer):
    ...
    def __init__(self, inplace=True, cmap=cv2.COLORMAP_PARULA, alpha=0.7):
        super(DensePoseResultsUVisualizer, self).__init__(
            _extract_u_from_iuvarr, _extract_i_from_iuvarr, inplace, cmap,
            alpha, val_scale=1.0)
...
class DensePoseResultsVVisualizer(DensePoseResultsVisualizer):
    ...
    def __init__(self, inplace=True, cmap=cv2.COLORMAP_PARULA, alpha=0.7):
        super(DensePoseResultsVVisualizer, self).__init__(
            _extract_v_from_iuvarr, _extract_i_from_iuvarr, inplace, cmap,
            alpha, val_scale=1.0)


class DensePoseOutputsSegmentationVisualizer(object):

    def __init__(self, inplace=True, cmap=cv2.COLORMAP_PARULA, alpha=0.7):
        self.mask_visualizer = MatrixVisualizer(
            inplace=inplace,
            cmap=cmap,
            val_scale=255.0 / (DensePoseDataRelative.N_BODY_PARTS + 1),
            alpha=alpha)

    def visualize(self, image_bgr, densepose_output_with_bboxes):
        densepose_output, bboxes_xywh = densepose_output_with_bboxes
        assert isinstance(densepose_output, DensePoseOutput), \
            'DensePoseOutput expected, {} encountered'.format(
        ...
        N = S.size(0)
        assert N == I.size(0), 'densepose outputs S {} and I {}' \
            ' should have equal first dim size'.format(S.size(), I.size())
        assert N == U.size(0), 'densepose outputs S {} and U {}' \
            ' should have equal first dim size'.format(S.size(), U.size())
        assert N == V.size(0), 'densepose outputs S {} and V {}' \
            ' should have equal first dim size'.format(S.size(), V.size())
        assert N == len(bboxes_xywh), 'number of bounding boxes {}' \
    ...
    def visualize(self, image_bgr, densepose_output_with_bboxes):
        ...
        for n in range(N):
            ...
            for partId in range(Un.shape[0]):
                Uvis[segmentation == partId] = Un[partId][
                    segmentation == partId].clip(0, 1) * 255
                bbox_xywh = bboxes_xywh[n]
            image_bgr = self.mask_visualizer.visualize(
                image_bgr, mask, Uvis, bbox_xywh)
        return image_bgr
    ...
    def visualize(self, image_bgr, densepose_output_with_bboxes):
        ...
        for n in range(N):
            ...
            for partId in range(Vn.size(0)):
                Vvis[segmentation == partId] = Vn[partId][
                    segmentation == partId].clip(0, 1) * 255
            bbox_xywh = bboxes_xywh[n]
            image_bgr = self.mask_visualizer.visualize(
                image_bgr, mask, Vvis, bbox_xywh)
        return image_bgr


class DensePoseDataSegmentationVisualizer(object):
    """
    ...
    def visualize(self, image_bgr, bbox_densepose_list):
        for bbox_xywh, densepose_data in zip(*bbox_densepose_list):
...
class DensePoseDataPointsVisualizer(object):
    ...
    def visualize(self, image_bgr, bbox_densepose_list):
        bbox_list, densepose_list = bbox_densepose_list
        for bbox_xywh, densepose_data in zip(*bbox_densepose_list):
...
def _densepose_data_i_for_cmap(densepose_data):
    i = np.clip(densepose_data.i.numpy(), 0.0, 25.0) * (255.0 / 25.0)
